...
void Predator::reprod(Predator * friends, vector<Predator> * newPred, Ecosystem * eco) {
	...
	if (this->getGender() == 1)//female. first parameter should be mother //MRE RandomGoodGene
		bebe->phGenome.crossover(this->phGenome.getGenome(), friends->phGenome.getGenome(), bebe_gender, 0);
	else
		bebe->phGenome.crossover(this->phGenome.getGenome(), friends->phGenome.getGenome(), bebe_gender, 0);
...
void Predator::act_driver(vector<Predator> * newPred, Ecosystem * eco, short MatingMode) {
	...
	setPosition(newPos);
	if (this->getGender() == 0){
		float newEnergy = calculationEnergyAction(getEnergy(), model.getNbArc(), getSpeed(), getStrength(), 0, eco);  //-- Losing Energy for acting //persuasion is used for prey deleterious mutation
		setEnergy(newEnergy);
	}
	else{
		float newEnergy = calculationEnergyAction(getEnergy(), model.getNbArc(), getSpeed(), getStrength(), 0, eco);  //-- Losing Energy for acting	//persuasion is used for prey deleterious mutation
											 ...
											 while (pasFait && int(localMaleFriends.size()) > 0) {
												 ...
												 if (getID() != eco->wolves[localMaleFriends[k]].getID()) {    //-- Preventing for mating with itself
													 ...
													 if (eco->wolves[localMaleFriends[k]].getUpdated() == 0) { //partner has not acted
														 ...
														 if ((eco->wolves[localMaleFriends[k]]).getAge() >= (eco->wolves[localMaleFriends[k]]).getRepAge() && eco->wolves[localMaleFriends[k]].getFCM()->ReviseReproductionDecision(0, eco)){ //partner want to reproduce
											 ...
											 while (pasFait && cntrLocalFriends <= int(localMaleFriends.size()) - 1) {
												 ...
												 if (getID() != eco->wolves[localMaleFriends[cntrLocalFriends]].getID()) {    //-- Preventing for mating with itself
													 ...
													 if (eco->wolves[localMaleFriends[cntrLocalFriends]].getUpdated() == 0) { //partner has not acted
														 ...
														 if ((eco->wolves[localMaleFriends[cntrLocalFriends]]).getAge() >= (eco->wolves[localMaleFriends[cntrLocalFriends]]).getRepAge() && eco->wolves[localMaleFriends[cntrLocalFriends]].getFCM()->ReviseReproductionDecision(0, eco)){ //partner want to reproduce
											 ...
											 while (pasFait && cntrLocalFriends <= int(LocalMaleFriends.size()) - 1) {
												 ...
												 if (getID() != eco->wolves[LocalMaleFriends[selectedRandomMateIndex]].getID()) {    //-- Preventing for mating with itself
													 ...
													 if (eco->wolves[LocalMaleFriends[selectedRandomMateIndex]].getUpdated() == 0) { //partner has not acted
														 ...
														 if ((eco->wolves[LocalMaleFriends[selectedRandomMateIndex]]).getAge() >= (eco->wolves[LocalMaleFriends[selectedRandomMateIndex]]).getRepAge() && eco->wolves[LocalMaleFriends[selectedRandomMateIndex]].getFCM()->ReviseReproductionDecision(0, eco)){ //partner want to reproduce
