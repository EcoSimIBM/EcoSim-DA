...
Ecosystem::Ecosystem() {
	...
	if (!paramMonde.is_open()) {		//-- Try to open parameters file
		...
		getline(paramMonde, entreInfo); words.clear(); Manip.Tokenize(entreInfo, words);
		persuasionHappens = 1; words.clear();
		...
		if (restore == 0) {
			...
			for (int i = 0; i < (initNbPrey / perClusterPrey); ++i) {		//-- Creates Prey
				...
				for (int j = 1; j <= perClusterPrey; j++) {
					...
					{
						newPrey = new Prey(type, ID, species, Parent1ID, Parent2ID, SpeciesParent1ID, SpeciesParent2ID, position, energy, energyMaxPrey, speed, speedMaxPrey, age, ageMax, vsionPrey, ageReprodPrey, DistEvolInitial, stateBirthPreyFemale, &fcm_prey, genderVal, 0.05, 0.05, 128); //Add Gender /MRE/
					}
					else
					{ //baby is male
						newPrey = new Prey(type, ID, species, Parent1ID, Parent2ID, SpeciesParent1ID, SpeciesParent2ID, position, energy, energyMaxPrey, speed, speedMaxPrey, age, ageMax, vsionPrey, ageReprodPrey, DistEvolInitial, stateBirthPreyMale, &fcm_prey, genderVal, 0.05, 0.05, 128); //Add Gender /MRE/
					}
...
void Ecosystem::updateEco() {
	...
	*/
	...
	for (size_t i = 0; i < rabbits.size(); ++i) {
		...
		//printf("\nPrey persuasion: %f %i\n", rabbits[i].getPersuasion(), (rabbits[i].getPhGenome())->persuasion);
	...
	for (unsigned int i = 0; i < rabbits.size(); i++) { // Act based on strength M.M; who has more strength act first
		...
		if (rabbits[i].getUpdated() == 0) {
			if (rabbits[i].get_action_offset() == FCMPrey::Reproduce){ //this is for the males who want to reproduce. since there is no female anymore this will fail...
				...
				//RS HERE
				float newEnergyPreyActed = rabbits[i].calculationEnergyAction(rabbits[i].getEnergy(), rabbits[i].model.getNbArc(), rabbits[i].getSpeed(), rabbits[i].getStrength(), rabbits[i].getPersuasionChar(), this);  //-- Losing Energy for acting //RS males don't carry persuasion penalty
	...
	*/
	...
	for (size_t i = 0; i < wolves.size(); ++i) {
		...
		//printf("\nPred persuasion: %f %i\n", wolves[i].getPersuasion(), (wolves[i].getPhGenome())->persuasion);
		//wolves[i].calcWeight();
	}
	...
	*/
	...
	for (size_t i = 0; i < rabbits.size(); ++i) {
		if (rabbits[i].getGender() == 1){
			...
			statWorld.incPersuasionFemalePrey(rabbits[i].getPersuasionChar());
		}
		else{
			...
			statWorld.incPersuasionMalePrey(rabbits[i].getPersuasionChar());
		}
	}
	...
	for (size_t i = 0; i < wolves.size(); ++i) {
		...
		if (wolves[i].getGender() == 1){
			...
			statWorld.incPersuasionFemalePred(0);
		}
		else{
			...
			statWorld.incPersuasionMalePred(0);
		}
	}
...
void Ecosystem::maxSave() {
	...
	for (size_t i = 0; i < rabbits.size(); i++) {
		...
		buffer += "19: " + Manip.vtos(rabbits[i].getPersuasionChar()) + "\n";
		stateFile << buffer;
		buffer.clear();
		
		//printf("\nPrey persuasion on MaxSave at generation %d: %i\n", generation, (rabbits[i].getPhGenome())->getPersuasionChar());
...
void Ecosystem::restoreState() {
	...
	*/
	...
	for (int i = 0; i < numPrey; i++) {
		...
		getline(paramMonde, entreInfo); Manip.Tokenize(entreInfo, words);
		persuasionVal = (unsigned short) atoi(words[1].c_str()); words.clear();
		...
		*/
		...
		//printf("\nPrey persuasion on restore at generation %d: %i %i\n", generation, persuasionVal, (newPrey->getPhGenome())->getPersuasionChar());
	...
	*/
	...
	for (int i = 0; i < numPred; i++) {
		...
		Predator * newPred = new Predator(t, pid, spec, p1id, p2id, p1sid, p2sid, p, e, eM, spd, sM, a, aM, v, rA, dE, sB, &fcm_pred, genderVal, 0); //Add Gender /MRE/
...
void Ecosystem::restoreState_HDF5() {
	...
	*/
	...
	for (int i = 0; i < numPred; i++) {
		...
		Predator * newPred = new Predator(t, pid, spec, p1id, p2id, p1sid, p2sid, p, e, eM, spd, sM, a, aM, v, rA, dE, sB, &fcm_pred, genderVal, 0);
