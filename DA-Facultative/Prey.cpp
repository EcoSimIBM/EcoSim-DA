...
void Prey::reprod(Prey * friends, vector<Prey> * newPrey, Ecosystem * eco) {
	...
	unsigned char persuasion;
	if (RandGen::next(32000) / 32000.0 <= 0.5)
		persuasion = this->phGenome.persuasion;
	else
		persuasion = friends->phGenome.persuasion;
	
	if (this->getGender() == 1)//female. first parameter should be mother //MRE RandomGoodGene
		bebe->phGenome.crossover(this->phGenome.getGenome(), friends->phGenome.getGenome(), bebe_gender, persuasion);
	else
		bebe->phGenome.crossover(this->phGenome.getGenome(), friends->phGenome.getGenome(), bebe_gender, persuasion);
	...
	float newEnergy = friends->calculationEnergyAction(friends->getEnergy(), friends->model.getNbArc(), friends->getSpeed(), friends->getStrength(), friends->getPersuasionChar(), eco); //-- Losing Energy for acting
...
void Prey::act_driver(vector<Prey> * newPrey, Ecosystem * eco, short MatingMode) {
	...
	setPosition(newPos);
	if (this->getGender() == 0){
		float newEnergy = calculationEnergyAction(getEnergy(), model.getNbArc(), getSpeed(), getStrength(), getPersuasionChar(), eco);  //-- Losing Energy for acting //Persuasion is being used just as deleterious mutation
		setEnergy(newEnergy);
	}
	else{
		float newEnergy = calculationEnergyAction(getEnergy(), model.getNbArc(), getSpeed(), getStrength(), getPersuasionChar(), eco);  //-- Losing Energy for acting	//Persuasion is being used just as deleterious mutation
										 ...
										 while (pasFait && int(LocalMaleFriends.size()) > 0) {
											 ...
											 if (getID() != eco->rabbits[LocalMaleFriends[k]].getID()) { //-- Preventing for mating with itself
												 ...
												 if (eco->rabbits[LocalMaleFriends[k]].getUpdated() == 0) { //partner has not acted
													 ...
													 if ((eco->rabbits[LocalMaleFriends[k]]).getAge() >= (eco->rabbits[LocalMaleFriends[k]]).getRepAge() && eco->rabbits[LocalMaleFriends[k]].getFCM()->ReviseReproductionDecision(0, eco)){ //partner want to reproduce
										 ...
										 while (pasFait && cntrLocalFriends <= int(LocalMaleFriends.size()) - 1) {
											 ...
											 if (getID() != eco->rabbits[LocalMaleFriends[cntrLocalFriends]].getID()) { //-- Preventing for mating with itself
												 ...
												 if (eco->rabbits[LocalMaleFriends[cntrLocalFriends]].getUpdated() == 0) { //partner has not acted
													 ...
													 if ((eco->rabbits[LocalMaleFriends[cntrLocalFriends]]).getAge() >= (eco->rabbits[LocalMaleFriends[cntrLocalFriends]]).getRepAge() && eco->rabbits[LocalMaleFriends[cntrLocalFriends]].getFCM()->ReviseReproductionDecision(0, eco)){ //partner want to reproduce
										 ...
										 while (pasFait && cntrLocalFriends <= int(LocalMaleFriends.size()) - 1) {
											 ...
											 if (getID() != eco->rabbits[LocalMaleFriends[selectedRandomMateIndex]].getID()) { //-- Preventing for mating with itself
												 ...
												 if (eco->rabbits[LocalMaleFriends[selectedRandomMateIndex]].getUpdated() == 0) { //partner has not acted
													 ...
													 if ((eco->rabbits[LocalMaleFriends[selectedRandomMateIndex]]).getAge() >= (eco->rabbits[LocalMaleFriends[selectedRandomMateIndex]]).getRepAge() && eco->rabbits[LocalMaleFriends[selectedRandomMateIndex]].getFCM()->ReviseReproductionDecision(0, eco)){ //partner want to reproduce
