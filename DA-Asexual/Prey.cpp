...
void Prey::reprod(Prey * friends, vector<Prey> * newPrey, Ecosystem * eco) {
	...
	float newEnergy = friends->calculationEnergyAction(friends->getEnergy(), friends->model.getNbArc(), friends->getSpeed(), friends->getStrength(), friends->getPersuasion(), eco); //-- Losing Energy for acting
