...
Sim::Sim(short type, long ID, int species, long parent1ID, long parent2ID, int speciesParent1ID, int speciesParent2ID, float energyMax, short speedMax, short ageMax, short vision, short repAge, float distanceEvol, float stateBirth, unsigned char nbGenes, short genderVal, float defence, float coopDefence, unsigned char persuasionVal) //Add Gender /MRE/
{
	...
	phGenome.setPersuasion(persuasionVal);
}
...
float Sim::calculationEnergyAction(float energy, int nbArc, float speed, float strength, unsigned char persuasion, Ecosystem * eco){
	...
	if (phGenome.getGenomeSize() > 6){ // prey
		...
		float energyToSpend = 0.9 * ((.8 * arcPenalty)  + ((strength*(pow((double)speed, 2))) / 10000.0) + pow((double)maxEnergy / 5.5, 0.75) + pow((double)vision * 5.0, 0.75) + (pow((double)maxSpeed * 5.0, 0.75)) + pow((double)defence * 100.0, 0.75) + pow((double)coopDefence * 75.0, 0.75) + pow((double)max(0.0, (double)(8 - repAge)), 2.3) + pow(persuasion * 3.0, 0.75));
		...
		float energyToSpend = 0.9 * ((.8 * arcPenalty)  + ((strength*(pow((double)speed, 2))) / 11000.0) + pow((double)maxEnergy / 5.5, 0.75) + pow((double)vision * 5.0, 0.75) + pow((double)maxSpeed * 5.0, 0.75) + pow((double)max(0.0, (double)(7 - repAge)), 2.3) + 0);
		e = energy - energyToSpend;
		if (getGender() == 0){
